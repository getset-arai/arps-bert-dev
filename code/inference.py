...
def model_fn(model_dir):
    ...
    pretrained_model_path = os.path.join(model_dir, "../model")
    pretrained_tokenizer_path = os.path.join(model_dir, "tokenizer")
    return BertProofreader(pretrained_model_path, pretrained_tokenizer_path)
...
def predict_fn(input_data, model):
    ...
    return model.check_toopk(input_data)


def output_fn(prediction, content_type=JSON_CONTENT_TYPE):
    return json.dump(prediction), content_type
